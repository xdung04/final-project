...
const fetchBranches = async () => {
  ...
  const handleMapUrlChange = (e) => {
    ...
    // Check nếu dán link chia sẻ dạng ngắn gọn (không có tọa độ)
    if (url.includes("goo.gl") || url.includes("maps.app.goo.gl")) {
      showToast("error", "Link rút gọn không chứa tọa độ. Vui lòng copy link dài trên thanh địa chỉ!");
      return;
    }

    // Regex bóc tách Vĩ độ và Kinh độ từ chuỗi chứa @lat,lng
    const regex = /@(-?\d+\.\d+),(-?\d+\.\d+)/;

...
export const vnpayReturn = async (req, res) => {
  ...
  try {
    ...
    if (isValid && responseCode === "00") {
      ...
      if (req.io) {
        // Bắn về luồng progress để màn hình Monitoring nhảy trạng thái thành công
        req.io.emit("receive_customer_progress", {
          bookingId: idBooking,
          step: 5, // Step 5 là step thành công/cảm ơn
          isPaid: true,
          method: "VNPAY",
          message: `Booking #${idBooking} đã thanh toán xong!`
        });
        
        // Hoặc bắn một sự kiện riêng nếu ông muốn Lễ tân hiện thông báo Toast

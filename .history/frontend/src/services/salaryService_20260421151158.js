...
/**
 * 8. Thợ lấy danh sách phiếu lương của chính mình (đã bỏ qua Draft)
 */
export const fetchMyPayslips = async (accessToken) => {
  try {
    const res = await request.get(`/salary/my-payslips`, {
      headers: { Authorization: `Bearer ${accessToken}` }
    });
    return res.data;
  } catch (error) {
    console.error("Lỗi khi fetch my payslips:", error);
    throw error.response?.data || error;
  }
};

/**
 * 9. Thợ xác nhận đồng ý phiếu lương
 */
export const confirmMyPayslip = async (idSalary, accessToken) => {
  try {
    const res = await request.patch(`/salary/${idSalary}/confirm`, {}, {
      headers: { Authorization: `Bearer ${accessToken}` }
    });
    return res;
  } catch (error) {
    console.error("Lỗi khi xác nhận phiếu lương:", error);
    throw error.response?.data || error;
  }
};

/**
 * 10. Thợ gửi khiếu nại phiếu lương
 */
export const disputeMyPayslip = async (idSalary, reason, accessToken) => {
  try {
    const res = await request.patch(`/salary/${idSalary}/dispute`, { reason }, {
      headers: { Authorization: `Bearer ${accessToken}` }
    });
    return res;
  } catch (error) {

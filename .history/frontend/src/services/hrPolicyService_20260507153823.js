...
export const getPromotionAlerts = async () => {
  try {
    return await request.get(`/hr-policy/promotion-alerts`);
  } catch (error) {
    throw error.response?.data || error;
  }
};
export const getSettlementPreview = async (idContract) => {
  return await request.get(`/hr-policy/contracts/${idContract}/settlement-preview`);
};
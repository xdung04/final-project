...
function BookingPage() {
  ...
  const handleBranchChange = async (e) => {
    ...
    setBranchDisplay(selectedBranch ? selectedBranch.name : "-- Chọn cơ sở --");
  ...
  const handleSubmit = async (e) => {
    ...
    // Ngăn chặn bấm nhiều lần khi đang loading
    if (isLoading) return; 
    ...
    try {
      ...
      setIsLoading(false); // Chỉ tắt loading khi có lỗi (vì thành công sẽ tự reload trang)
    }
  };
  ...
  return (
    <DefaultLayout>
            ...
            <div className={styles.formGroup}>
              ...
              <select value={booking.branchId || ""} onChange={handleBranchChange}>
                ...
                {branches.map((b) => {
                  ...
                  let label = b.name;
                  if (suspend && !resume) label += ` — (sẽ ngưng hoạt động từ ${suspend})`;
                  if (resume && new Date(b.resumeDate) > new Date()) label += ` — (sẽ hoạt động từ ${resume})`;
              ...
              <ul className={styles.serviceList}>
                {booking.services.map((s) => (
                  <li key={s.idService}>
                    ...
                    <button type="button" onClick={() => handleRemoveService(s.idService)}>X</button>
                  </li>
                ))}
              </ul>
            </div>
      ...
      {/* OVERLAY LOADING FULL MÀN HÌNH */}
      {isLoading && (
        <div className={styles.overlay}>
          <div className={styles.loader}></div>
          <p className={styles.loadingText}>Đang hoàn tất lịch hẹn...</p>
        </div>
      )}
    </DefaultLayout>
  );

...
// ─── Ước tính thời gian lái xe nội thành (≈ 25 km/h) ──────────────────────
...
function BookingPage() {
  ...
  // ─── Branches đã sort theo khoảng cách ───────────────────────────────────
  // Giả sử API trả về mỗi branch có field `lat` và `lng`.
  // Nếu API chưa có, bạn thêm 2 field đó vào response BE hoặc dùng bảng tĩnh.
  ...
  const nearestBranchId = branchesWithDistance[0]?.idBranch ?? null;
        ...
        <div className={styles.bookingContainer}>
          ...
          <div className={styles.branchFinder}>
            ...
            <div className={styles.branchCardList}>
              {branchesWithDistance.map((b, idx) => {
                const isNearest = userLocation && b.distance != null && idx === 0;
                ...
                return (
                  ...
                  >
                    ...
                    {b.address && (
                      <div className={styles.branchCardAddr}>{b.address}</div>
                    )}
          ...
          <form onSubmit={handleSubmit}>
            {/* Cơ sở (dropdown vẫn giữ để backup) */}

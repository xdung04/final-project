...
const handleAddressChange = (e) => {
    ...
    const results = [];
    allWards.forEach((w) => {
      // Tìm Huyện và Tỉnh tương ứng với Phường/Xã hiện tại
      ...
      // Ghép thành một chuỗi địa chỉ hoàn chỉnh (VD: Phường Bến Nghé, Quận 1, Thành phố Hồ Chí Minh)
      const fullAddress = `${w.name}, ${district?.name}, ${province?.name}`;

      // TÌM KIẾM TỪ KHÓA TRONG TOÀN BỘ CHUỖI ĐỊA CHỈ
      if (normalize(fullAddress).includes(actualKeyword)) {
        results.push(`${street ? street + ", " : ""}${fullAddress}`);
      }
    });

    // Giới hạn hiển thị 10 kết quả để tránh lag
...
const handleMapUrlChange = (e) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      // Đọc lỗi từ Backend trả về (nếu có), nếu không thì hiện lỗi mặc định

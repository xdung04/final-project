...
import { SalaryAPI } from "~/apis/salaryAPI"; // Đảm bảo đúng tên file api của ông nhé
...
const calcNet = (s) =>
  (Number(s.baseSalary)||0) + (Number(s.commission)||0) + (Number(s.tip)||0) + (Number(s.bonus)||0)
  - (Number(s.advance)||0) - (Number(s.deduction)||0);
    ...
    <Modal open={!!data} onClose={onClose}>
      ...
      <div className={cx("previewBox")}>
        ...
        <span className={cx("previewVal")}>
          {fmt((Number(data?.baseSalary)||0) + (Number(data?.commission)||0) + (Number(data?.tip)||0) + (Number(data?.bonus)||0) - Number(advance) - Number(deduction))}
        </span>
      </div>
...
function PayslipModal({ data, onClose }) {
  if (!data) return null;
  const totalIncome = (Number(data.baseSalary)||0) + (Number(data.commission)||0) + (Number(data.tip)||0) + (Number(data.bonus)||0);
  const totalDeduct = (Number(data.advance)||0) + (Number(data.deduction)||0);
...
function LuongThuong() {
  ...
  const [canCalculate, setCanCalculate] = useState(false); // Cờ kiểm soát nút Tính Lương từ Backend
  ...
  const fetchData = useCallback(async (m, y) => {
    ...
    try {
      ...
      // Backend mới trả về dạng { source, canCalculate, salaries }
      // Nếu API lỗi hoặc chưa đồng bộ kịp thì fallback dùng mảng bình thường
      const dataList = res?.salaries || (Array.isArray(res) ? res : []);
      
      setSalaries(dataList);
      
      // Nếu backend có trả về cờ canCalculate thì dùng, không thì tự check (nếu không phải tháng hiện tại)
      if (res?.hasOwnProperty('canCalculate')) {
        setCanCalculate(res.canCalculate);
      } else {
        const isCurrentMonth = m === today.getMonth() + 1 && y === today.getFullYear();
        setCanCalculate(!isCurrentMonth);
      }
  ...
  const handleCalculateDraft = () => {
    withLoading(async () => {
      try {
        ...
        alert("Có lỗi xảy ra khi lưu nháp lương, vui lòng kiểm tra lại Backend!");
      }
    });
  };
  ...
  const handleForceClose = (reason) => withLoading(async () => {
    // Sửa đúng tên hàm forceCloseDispute
    ...
    <div className={cx("page")}>
      ...
      <div className={cx("header")}>
        <div className={cx("headerLeft")}>
          ...
          <h1 className={cx("headerTitle")}>Lương &amp; Hoa hồng Barber</h1>
        </div>
      ...
      <div className={cx("actionBar")}>
        <div className={cx("actionGroup")}>
          <button
            ...
            title={!canCalculate ? "Không thể lưu nháp lúc này (Có thể do đang là tháng hiện tại hoặc dữ liệu đã được lưu)" : ""}
          >
            ...
            <table className={cx("table")}>
              ...
              <tbody>
                {filtered.length > 0 ? filtered.map((s, idx) => {
                  const totalIncome = (Number(s.baseSalary)||0) + (Number(s.commission)||0) + (Number(s.tip)||0) + (Number(s.bonus)||0);
                  const totalDeduct = (Number(s.advance)||0) + (Number(s.deduction)||0);
                  const net = totalIncome - totalDeduct;

...
const fetchData = useCallback(async (m, y) => {
    setLoading(true);
    try {
      // Gọi API lấy dữ liệu của tháng/năm cụ thể
      const res = await SalaryAPI.getSalaries(m, y);
      
      // Kiểm tra xem res trả về là gì để setSalaries cho đúng
      // Nếu API trả về trực tiếp mảng danh sách thợ: dùng res
      // Nếu API trả về object có chứa mảng { salaries: [...] }: dùng res.salaries
      setSalaries(Array.isArray(res) ? res : (res?.salaries || []));
...
const handleCalculateDraft = () => {
    withLoading(async () => {
      try {
        // 1. Gọi API tính lương
        await SalaryAPI.createDraftSalaries(month, year);
        
        // 2. Chờ một chút (nếu cần thiết để DB kịp commit) 
        // Nếu API nhanh, bạn có thể bỏ dòng này.
        await new Promise(resolve => setTimeout(resolve, 500));
        
        // 3. Tải lại dữ liệu
        await fetchData(month, year);
        
        // 4. Kiểm tra logs để xem dữ liệu có thực sự về không
        console.log("Dữ liệu sau khi tính lương:", salaries);
        
      } catch (err) {
        console.error("Lỗi khi tính lương:", err);
        alert("Có lỗi xảy ra khi tính lương, vui lòng kiểm tra lại Backend!");
      }
    });
  };

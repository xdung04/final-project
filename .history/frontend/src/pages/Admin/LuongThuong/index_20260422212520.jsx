...
// ─── Source Banner ────────────────────────────────────────────────────────────
// Hiển thị khi dữ liệu đến từ realtime (chưa lưu DB)
...
function DeductionModal({ data, onClose, onSave, loading }) {
  ...
  const previewNet =
    ...
    Number(advance) - Number(deduction);
    ...
    <Modal open={!!data} onClose={onClose}>
      ...
      <div className={cx("formGroup")}>
        ...
        <textarea
          ...
          value={note} placeholder="Lý do điều chỉnh..."
...
function DisputeViewModal({ data, onClose, onForceClose, onEditAndResend, loading }) {
  ...
  // Reset reason mỗi lần mở modal mới
...
function LuongThuong() {
  ...
  // Metadata từ response backend
  const [dataSource, setDataSource]       = useState("realtime"); // "database" | "realtime"
  ...
  const fetchData = useCallback(async (m, y) => {
    setLoading(true);
    try {
      // Backend trả về: { source, isCurrentMonth, canCalculate, message, salaries }
      ...
      // Cập nhật danh sách lương — luôn là mảng
  ...
  // Đếm theo status — Realtime không tính vào workflow counts
  const counts = {
  ...
  // Chỉ active khi canCalculate = true (backend quyết định)
  const handleCalculateDraft = () =>
    withLoading(async () => {
      await SalaryAPI.createDraftSalaries(month, year);
      await fetchData(month, year); // Reload → sẽ lấy từ DB lúc này
    });
  ...
  // ── Action: Lưu điều chỉnh khấu trừ ────────────────────────────────────
  // SalaryAPI.adjustSalary(idSalary, payload) — 2 tham số riêng
  const handleSaveDeduction = (payload) =>
    withLoading(async () => {
      const { idSalary, ...adjustData } = payload;
      await SalaryAPI.adjustSalary(idSalary, adjustData);
      setEditingRow(null);
      await fetchData(month, year);
    });

  // ── Action: Force-close khiếu nại ───────────────────────────────────────
  // Đúng tên API: forceCloseDispute (không phải forceCloseSalaryDispute)
  ...
  const handleEditAndResend = () => {
    ...
    setDisputeRow(null);
    setEditingRow(row); // Mở DeductionModal với dữ liệu hiện tại
  };

  // ── Action: Thanh toán 1 phiếu ──────────────────────────────────────────
  // SalaryAPI.markAsPaid(idSalary, { paidAmount, paymentProofUrl })
  ...
  const renderActions = (s) => {
    // Realtime: chỉ được xem, không có action nào
    ...
    return (
      <div className={cx("actionCell")}>
        {/* Nút xem khiếu nại — chỉ khi Disputed */}
        {s.status === "Disputed" && (
          <button
        ...
        {/* Nút chỉnh khấu trừ — Draft hoặc Disputed */}
        {canEdit && !locked && (
          <button
            className={cx("iconBtn")}
        ...
        {/* Nút gửi lại — sau khi sửa Disputed */}
        {canResend && (
          <button
            className={cx("iconBtn", "send")}
            onClick={() => handleSendOne(s)}
  ...
  return (
    <div className={cx("page")}>
      ...
      {/* ── Source Banner — hiện khi dữ liệu là realtime ── */}
      <SourceBanner
        source={dataSource}
      ...
      {/* ── Workflow Bar — chỉ hiện khi có data từ DB ── */}
      {dataSource === "database" && (
      ...
      <div className={cx("actionBar")}>
        <div className={cx("actionGroup")}>
          {/*
           * Nút "Tính lương nháp":
           * - Disabled khi: tháng hiện tại, đang loading, hoặc canCalculate = false
           * - canCalculate do backend quyết định (tháng trước + chưa có DB)
           */}
          <button
      ...
      {/* ── Stat Cards — chỉ hiện khi có data DB ── */}
      {dataSource === "database" && (
      ...
      <div className={cx("tableSection")}>
        <div className={cx("tableHeader")}>
          ...
          <p className={cx("tableDesc")}>
            Thực nhận = (Cơ bản + Hoa hồng + Tip + Thưởng) − (Tạm ứng + Phạt)
          </p>
        </div>

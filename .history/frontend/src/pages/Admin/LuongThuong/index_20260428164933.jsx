...
function DeductionModal({ data, onClose, onAdd, onRemove, loading }) {
  ...
  const [removingId, setRemovingId]     = useState(null); // idDeduction đang mở form xóa

  // Reset form mỗi lần mở modal mới (track bằng idSalary để tránh reset khi re-render)
  useEffect(() => {
    if (data) {
      setAmount("");
      setReason("");
  ...
  // Preview dựa trên deductions cached column (đã được recalculate ở backend)
  const previewNet =
  ...
  const handleAdd = () => {
    ...
    onAdd(data.idSalary, { amount: Number(amount), reason: reason.trim() });
    setAmount("");
    setReason("");
  };
...
function LuongThuong() {
  ...
  const handleAddDeduction = (idSalary, { amount, reason }) =>
    withLoading(async () => {
      await SalaryAPI.addDeduction(idSalary, { amount, reason });
      // Reload để DeductionsList + deductions cached column cập nhật
      const res = await SalaryAPI.getSalaries(month, year);
      setSalaries(Array.isArray(res.salaries) ? res.salaries : []);
      // Đồng bộ lại editingRow với data mới nhất để list trong modal cập nhật

...
function ContractModal({ mode, barber, plans, onSave, onClose }) {
  const [form, setForm] = useState({
    ...
    // startDate tự động = mùng 1 tháng sau, không cho sửa
    startDate: getFirstDayNextMonth(),
  });
      ...
      <div className={cx("modalBox")}>
        ...
        <div className={cx("modalFooter")}>
          ...
          <button className={cx("btn", "gold")}
            onClick={() => onConfirm({
              idContract:         promotionItem.idSalaryContract,
...
function SettlementModal({ barber, onConfirm, onClose }) {
  ...
  const addDeduction = () => {
    setDeductions((d) => [...d, { amount: 0, reason: "", violationDate: "" }]);
  };
  ...
  return (
    <div className={cx("modalOverlay")} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className={cx("modalBox", "modalBoxWide")}>
        ...
        <div className={cx("modalBody")}>
          {/* Thu nhập — Backend tính, FE chỉ hiển thị sau khi submit */}
          <div className={cx("settlementNote")}>
            <AlertTriangle size={13} />
            Lương thực tế sẽ được tính tự động theo số ngày làm việc đến{" "}
            <strong>{fmtDate(barber.endDate)}</strong>.
          </div>

          {/* Khấu trừ */}
          <div className={cx("sectionSubTitle")}>Khấu trừ</div>
          {deductions.map((d, idx) => (
            <div key={idx} className={cx("deductionRow")}>
              ...
              <button className={cx("btn", "iconBtn", "danger")}
          ...
          <button className={cx("btn", "outline", "small")} onClick={addDeduction}>
            <Plus size={13} /> Thêm khấu trừ
          </button>

          {/* Tổng khấu trừ preview */}
          {totalDeductions > 0 && (
            <div className={cx("settlementTotal")}>
              Tổng khấu trừ: <strong>-{fmt(totalDeductions)}đ</strong>
            </div>
          )}
        </div>
        ...
        <div className={cx("modalFooter")}>
          ...
          <button className={cx("btn", "danger")}
            onClick={() => onConfirm(barber.idSalaryContract, deductions)}>
  ...
  const handleAction = async (action, barber) => {
    switch (action) {
      ...
      case "cancel_pending": {
        ...
        try {
          // Hủy HĐ chưa hiệu lực = xóa hoặc terminate tùy backend
          await HrPolicyAPI.settleContract(barber.idSalaryContract, []);

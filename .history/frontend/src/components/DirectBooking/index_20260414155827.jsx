          ...
          <div className={styles.popupOverlay}>
            <div className={styles.popupContent}>
              ...
              <div className={styles.popupBtnRow}>
                <button className={styles.cancelBtn} onClick={() => setShowCreateCustomer(false)}>Hủy bỏ</button>
                <button className={styles.createBtn} onClick={handleCreateCustomer}>Tạo tài khoản</button>
              </div>
            </div>
          </div>
        )}
        ...
        <div className={styles.scrollable}>
          <div className={styles.formContent}>
            ...
            <div className={styles.section}>
              ...
              <select value={form.date} onChange={handleDateChange} disabled={!form.barberId}>
            ...
            <div className={styles.section}>
              ...
              <div className={styles.timeGrid}>
                {times.length === 0 && <span style={{color: '#a3a3a3', fontSize: '13px'}}>Vui lòng chọn chi nhánh và ngày.</span>}
                {times.map((time, i) => {
                  let isPast = false;
                  if (form.date) {
            ...
            <div className={styles.section}>
              ...
              <select onChange={handleServiceAdd} value="" disabled={!form.branchId}>
                  ...
                  {form.services.map((s) => (
                    <li key={s.idService}>
                      ...
                      <div style={{display: 'flex', gap: '15px', alignItems: 'center'}}>
                        <span style={{color: '#b8966a'}}>{Number(s.price).toLocaleString()}đ</span>
            ...
            <div className={`${styles.section} ${styles.priceBox}`}>
              <p>TỔNG THANH TOÁN: <strong>{totalPrice.toLocaleString("vi-VN")} VNĐ</strong></p>
            </div>
          </div>
        </div>

        <div className={styles.submitContainer}>
          <button className={styles.submitBtn} onClick={handleSubmit}>Xác nhận & Đặt lịch</button>
        </div>
      </div>
    </div>
  );
...
const MonitoringView = ({ guestProgress, onCancel, onCashPayment }) => {
  const getStepText = (step) => {
    const steps = {
      ...
      6: "Khách yêu cầu thanh toán TIỀN MẶT",   // ← Mới thêm
    };
  ...
  return (
    <div className={monitorStyles.monitoringContent}>
      ...
      <div className={monitorStyles.liveStats}>
        ...
        {/* Hiển thị thêm thông tin khi khách chọn tiền mặt */}
        {guestProgress.step === 6 && (
          <div className={monitorStyles.statItem} style={{ gridColumn: "1 / -1" }}>
            <label>Tổng tiền cần thu</label>
            <span className={monitorStyles.value} style={{ color: "#d4af77", fontSize: "1.6rem" }}>
...
export default function ReceptionistPayment({ booking, onClose, onPushSuccess }) {
  ...
  useEffect(() => {
    ...
    // Lắng nghe tiến độ chung từ khách (Rating, Tip, Invoice...)
    socket.on("receive_customer_progress", (data) => {
      setGuestProgress(data);
    });

    // ←←← SỬA Ở ĐÂY: Lắng nghe khi khách bấm "Thanh toán tiền mặt"
    socket.on("customer_choose_cash_payment", (data) => {
      setGuestProgress({
        step: 6,                    // Bước mới dành riêng cho tiền mặt
        rating: data.rating || guestProgress.rating,
        tip: data.tip || 0,
        total: data.total || 0,
        isCashPayment: true,
      });

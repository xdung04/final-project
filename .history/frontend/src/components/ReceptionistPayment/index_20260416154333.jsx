...
import styles from "./PaymentModal.module.scss"; // Bạn vẫn nên để file SCSS riêng cho đẹp
  ...
  return (
    <div className={monitorStyles.monitoringContent}>
      ...
      <div className={monitorStyles.liveStats}>
        <div className={monitorStyles.statItem}>
          ...
          <span className={monitorStyles.stars}>
            {guestProgress.rating > 0 ? "⭐".repeat(guestProgress.rating) : "---"}
          </span>
        </div>
        <div className={monitorStyles.statItem}>
      ...
      <div className={monitorStyles.actionGroup}>
        <button onClick={onCancel} className={monitorStyles.btnCancel}>✕ Hủy / Rút Bill về</button>
        <button onClick={onCashPayment} className={monitorStyles.btnCash}>💵 Thu tiền mặt / Hoàn tất</button>
      </div>
    </div>
  );
};

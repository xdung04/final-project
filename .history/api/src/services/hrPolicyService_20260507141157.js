...
export const getPromotionAlerts = async () => {
  ...
  const alerts = await Promise.all(
    notifications.map(async (n) => {
      const contract = await db.SalaryContract.findByPk(n.referenceId, {
        include: [
          {
            model: db.CompensationPlan,
            as: "plan",
      ...
      // ✅ Lấy kỳ lương Confirmed gần nhất của barber
      const lastConfirmedSalary = await db.Salary.findOne({
        where: {
          idBarber:        contract.idBarber,
      ...
      // Chưa có lương confirmed → bỏ qua
      if (!lastConfirmedSalary) return null;

      return {
        idNotification:    n.idNotification,
        idContract:        contract.idSalaryContract, // ✅ tên JS
        idBarber:          contract.idBarber,
        barberName:        contract.barber?.user?.fullName || "",
        currentPlanName:   contract.plan?.displayName     || "",
        nextPlanName:      contract.plan?.nextPlan?.displayName || "",
        idNextPlan:        contract.plan?.idNextPlan,
        defaultBaseSalary: contract.plan?.nextPlan?.defaultBaseSalary || 0,
        // ✅ salaryPeriod từ DB — chính xác
        salaryPeriod: {
          month: lastConfirmedSalary.month,
          year:  lastConfirmedSalary.year,
        },
      };
    })
  );

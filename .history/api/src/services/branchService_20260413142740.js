...
export const createBranch = async (data) => {
  try {
    const { 
      name, 
      address, 
      latitude,      // Tọa độ vĩ độ từ Frontend
      longitude,     // Tọa độ kinh độ từ Frontend
      openTime, 
      closeTime, 
      slotDuration, 
      selectedServices,
    ...
    // Kiểm tra tính hợp lệ của tọa độ (nếu có gửi lên)
    ...
    // ---- 2. XỬ LÝ NGÀY KHAI TRƯƠNG (startDate) ----
    ...
    // ---- 3. TẠO CHI NHÁNH MỚI TRONG DATABASE ----
    const newBranch = await Branch.create({
      name,
      address,
      latitude: latitude || null,   // Lưu tọa độ
      longitude: longitude || null, // Lưu tọa độ
      openTime,
      closeTime,
      slotDuration,
      status: "Inactive",           // Mặc định Inactive cho đến ngày khai trương
      resumeDate: inputDate,        // Ngày dự kiến mở cửa
    });

    // ---- 4. GÁN DỊCH VỤ CHO CHI NHÁNH (Many-to-Many) ----
    if (Array.isArray(selectedServices) && selectedServices.length > 0) {
      // Sequelize tự động tạo các bản ghi trong bảng ServiceAssignment
      await newBranch.addServices(selectedServices);
    }

    // ---- 5. LẤY LẠI DỮ LIỆU ĐÃ TẠO KÈM DỊCH VỤ ĐỂ TRẢ VỀ FRONTEND ----
    const branchWithServices = await Branch.findByPk(newBranch.idBranch, {
      include: [{
        model: Service,
        as: "services",
        attributes: ["idService", "name", "price", "duration", "status"],
        through: { attributes: [] }, // Ẩn các trường trung gian của bảng nối
      }],
    });
    ...
    console.error("Lỗi tại createBranch Service:", error.message);
    throw error;
  }
};

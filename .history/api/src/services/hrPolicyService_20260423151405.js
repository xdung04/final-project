...
export const getAllActivePlans = async () => {
  try {
    const plans = await db.CompensationPlan.findAll({
      where: { effectiveTo: null }, // Chỉ lấy các plan đang có hiệu lực
...
export const getBarbersWithContracts = async () => {
  try {
    const barbers = await db.Barber.findAll({
      ...
      include: [
        { model: db.User, as: "user", attributes: ["fullName", "phone", "email"] }, // Giả định ông có join bảng User để lấy tên
        {
          ...
          include: [
            {
              model: db.CompensationPlan,
              as: "plan",
              attributes: ["idCompensationPlan", "displayName", "defaultBaseSalary", "color"],
            },
          ],
        },
      ],

...
export const getMyPayslips = async (idUser) => {
  // Tìm idBarber từ idUser thông qua bảng Barber
  const barber = await db.Barber.findOne({ where: { idUser: parseInt(idUser) } });
  if (!barber) throw new Error("Không tìm thấy hồ sơ thợ cắt tóc.");

  // Lấy danh sách lương, dùng Op.ne để lọc bỏ "Draft"
  const payslips = await db.Salary.findAll({
    where: {
      idBarber: barber.idBarber,
      status: { [Op.ne]: "Draft" } // Thợ không được xem bản nháp
    },
    include: [
      {
        model: db.SalaryDeduction,
        as: "DeductionsList" // Để lấy chi tiết Tạm ứng/Phạt
      }
    ],
    order: [
      ["year", "DESC"],

...
export const checkPromotionEligibility = async (idBarber) => {
  ...
  await notifyPromotionEligible({
    ...
    idContract:      activeContract.idContract, // ✅ đúng field name
  });
};
// BE: hrPolicyService.js
export const getPromotionAlerts = async () => {
  // Lấy notification SYSTEM chưa đọc liên quan promotion
  ...
  // Với mỗi notification, lấy thêm data từ contract
  ...
  return alerts.filter(Boolean); // bỏ null
};
...
export const getBarberBookingsNext7Days = async (idBarber) => {
  ...
  // 🔴 NẾU THỢ BỊ KHÓA: Trả về mảng booking rỗng + 1 cục ngày nghỉ GIẢ kín 7 ngày
  if (barber.isLocked) {
    return { 
      bookings: [], 
      // Tạo cục data này để Frontend tự động block UI không cho bấm
      unavailabilities: [
        {
          idUnavailable: 999999, // Cứ cho 1 ID bừa nào đó
          startDate: today,
          endDate: next7Days,
          reason: "Tài khoản thợ đang tạm khóa"
        }
      ] 
    };
  }

  // 🟢 NẾU THỢ BÌNH THƯỜNG: Lấy lịch booking như cũ
  const bookings = await db.Booking.findAll({
    where: { 
      idBarber,
  ...
  // Trả về unavailabilities: [] để Frontend không bị lỗi "undefined"

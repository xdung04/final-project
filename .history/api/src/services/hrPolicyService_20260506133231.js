...
export const promoteBarber = async (idBarber, contractData) => {
  const t = await db.sequelize.transaction();
  try {
    // ✅ Validate plan moi ton tai truoc khi lam gi ca
    const newPlan = await db.CompensationPlan.findByPk(
      contractData.idCompensationPlan,
      { transaction: t }
    );
    if (!newPlan) throw new Error("Plan moi khong ton tai.");
    if (newPlan.effectiveTo !== null) throw new Error("Plan moi da het hieu luc.");
    ...
    // ✅ Khong len cap neu HD cu da co endDate (dang trong lo trinh nghi viec)
    if (activeContract.endDate) {
      throw new Error("Barber nay da duoc thiet lap ngay nghi viec, khong the len cap.");
    }

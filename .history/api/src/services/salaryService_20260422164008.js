...
export const getSalariesUnified = async (month, year) => {
  ...
  const savedSalaries = await db.Salary.findAll({
    ...
    include: [
      { 
        model: db.Barber, as: "barber", 
        include: [{ model: db.User, as: "user" }, { model: db.Branch, as: "branch" }] 
      },
      { model: db.SalaryDeduction, as: "DeductionsList" }
    ],
  });
  ...
  if (savedSalaries && savedSalaries.length > 0) {
    console.log(`>>> Lấy dữ liệu từ DB cho tháng ${month}/${year}`);
  ...
  // 2. Nếu DB trống -> Tính Real-time (Bỏ qua check isCurrentMonth để test cho dễ)
  console.log(`>>> DB trống, đang tính Real-time cho tháng ${month}/${year}...`);
  const rawData = await getBarberSalariesOptimized(month, year);
  
  console.log(">>> Kết quả rawData từ hàm Optimize:", rawData.length, "bản ghi");
  ...
  return rawData.map(item => ({
    ...
    advance: 0,
    deduction: 0,
    status: "Draft" // Tự hiểu là nháp khi chưa lưu
  }));
};

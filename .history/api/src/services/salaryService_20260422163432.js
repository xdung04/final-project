...
export const getSalariesUnified = async (month, year) => {
  // 1. Luôn thử lấy từ DB trước (dù tháng nào cũng vậy)
  const savedSalaries = await db.Salary.findAll({
    where: { month, year },
    include: [
      { model: db.Barber, as: "barber", include: [{ model: db.User, as: "user" }, { model: db.Branch, as: "branch" }] },
      { model: db.SalaryDeduction, as: "DeductionsList" }
    ],
  });

  // 2. Nếu đã có dữ liệu trong DB -> Trả về ngay
  if (savedSalaries && savedSalaries.length > 0) {
    return savedSalaries.map(s => {
      ...
      return {
        ...
        tip: parseFloat(s.tips),
        bonus: parseFloat(s.bonus),
        serviceRevenue: 0, // Dữ liệu đã lưu thường không cần giữ revenue gốc, trừ khi bạn cần
        advance,
        deduction,
        adjustmentNote: s.adjustmentNote,
        status: s.status,
  ...
  // 3. Nếu DB trống, mới tiến hành tính toán Real-time
  // Chỉ nên cho phép tính real-time nếu là tháng hiện tại hoặc tháng trước
  const rawData = await getBarberSalariesOptimized(month, year);
  
  // Chuẩn hóa dữ liệu Real-time để có cấu trúc Y HỆT DB ở trên
  return rawData.map(item => ({
    idSalary: null, // Quan trọng: null để Frontend biết là chưa lưu
    idBarber: item.idBarber,
    ...
    advance: 0,           // Mặc định
    deduction: 0,         // Mặc định
    adjustmentNote: "",   // Mặc định
...
export const adjustSalary = async ({ idSalary, advance, deduction, adjustmentNote }) => {
  const transaction = await db.sequelize.transaction();
  console.log("Đang tìm phiếu lương với ID:", idSalary);
  try {

...
export const getMyPayslips = async (idUser) => {
  // Tìm idBarber từ idUser
  const barber = await prisma.barber.findUnique({ where: { idUser: parseInt(idUser) } });
  if (!barber) throw new Error("Không tìm thấy hồ sơ thợ cắt tóc.");

  // Lấy danh sách lương, LỌC BỎ status "Draft"
  const payslips = await prisma.salary.findMany({
    where: {
      idBarber: barber.idBarber,
      status: { not: "Draft" } // Quan trọng: Thợ không được xem bản nháp
    },
    orderBy: [
      { year: 'desc' },
      { month: 'desc' }
...
export const confirmPayslipByBarber = async (idSalary, idUser) => {
  const salary = await prisma.salary.findUnique({ 
    where: { idSalary: parseInt(idSalary) },
    include: { barber: true }
  });

  if (!salary) throw new Error("Không tìm thấy phiếu lương.");
  if (salary.barber.idUser !== parseInt(idUser)) throw new Error("Bạn không có quyền thao tác trên phiếu lương này.");
  if (salary.status !== "Pending") throw new Error("Chỉ có thể xác nhận phiếu lương ở trạng thái Chờ (Pending).");
  ...
  await createNotification({ type: "SALARY", targetRole: "admin", content: `Thợ ${salary.barber.name} đã xác nhận lương tháng ${salary.month}.` });
...
export const disputePayslipByBarber = async (idSalary, idUser, reason) => {
  ...
  if (!salary) throw new Error("Không tìm thấy phiếu lương.");
  if (salary.barber.idUser !== parseInt(idUser)) throw new Error("Bạn không có quyền thao tác trên phiếu lương này.");
  if (salary.status !== "Pending") throw new Error("Chỉ có thể khiếu nại phiếu lương ở trạng thái Chờ (Pending).");
  ...
  const updatedSalary = await prisma.salary.update({
    ...
    data: { 
      ...
      disputeCount: { increment: 1 } // Tăng số lần khiếu nại lên 1
    }
  });

  // (Tuỳ chọn) Bắn Notification khẩn cho Admin
  // await createNotification({ type: "SALARY_DISPUTE", targetRole: "admin", content: `Thợ ${salary.barber.name} vừa khiếu nại lương tháng ${salary.month}!` });

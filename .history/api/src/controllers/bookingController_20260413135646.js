...
export const createBooking = async (req, res) => {
  try {
    // Middleware 'authenticate' đã gán thông tin này vào req.user
    const idCustomer = req.user.idUser;
    const userRole = req.user.role;

    // 🔥 CHẶN BIẾN THỂ: Chỉ 'customer' mới được dùng API này
    if (userRole !== 'customer') {
      return res.status(403).json({
        success: false,
        message: "Tính năng này chỉ dành cho khách hàng. Nhân viên vui lòng sử dụng giao diện quản lý nội bộ.",
      });
    }

    const bookingData = {
      ...req.body,
      idCustomer, // Gán id từ token để đảm bảo an toàn, không lấy id từ body
    };
    ...
    console.error("Lỗi tại createBooking Controller:", error);

...
// 🔥 THÊM MỚI: Xóa Plan (soft-delete bằng SCD Type 2)
export const deletePlan = async (req, res) => {
  try {
    const { idPlan } = req.params;
    await HrPolicyService.deletePlan(idPlan);
    return res.status(200).json({ message: "Đã xóa cấp bậc thành công" });
  } catch (error) {
    console.error("Lỗi deletePlan:", error);
    // Còn contract active dùng Plan này → 409 Conflict
    const statusCode = error.message.includes("Đang có thợ sử dụng") ? 409 : 500;
    return res.status(statusCode).json({ message: error.message });
  }
};

// 🔥 THÊM MỚI: Clone Plan (khi Plan đã bị lock bởi Salary)
export const clonePlan = async (req, res) => {
  try {
    const { idPlan } = req.params;
    const newPlan = await HrPolicyService.clonePlan(idPlan);
    return res.status(201).json({
      message: "Đã tạo bản sao Plan thành công. Vui lòng chỉnh sửa rules trên Plan mới.",
      data: newPlan
    });
  } catch (error) {
    console.error("Lỗi clonePlan:", error);
    return res.status(500).json({ message: "Lỗi server khi clone Plan", error: error.message });
  }
};
...
export const saveCommissionRules = async (req, res) => {
  try {
    ...
    // Plan đã bị lock bởi Salary → 409 Conflict
    const statusCode = error.message.includes("đã được dùng để tính lương") ? 409 : 500;
    return res.status(statusCode).json({ message: error.message });
  }
};
...
export const updatePendingContract = async (req, res) => {
  try {
    ...
    const statusCode = error.message.includes("không được chỉnh sửa") ? 403 : 500;
    return res.status(statusCode).json({ message: error.message });
  }
};

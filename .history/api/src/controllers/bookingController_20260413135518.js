...
export const createBooking = async (req, res) => {
  try {
    ...
    const userRole = req.user.role; // Giả định middleware auth đã giải mã role từ token

    // 🔥 BƯỚC KIỂM TRA QUAN TRỌNG: Chỉ cho phép 'customer' đặt lịch
    if (userRole !== 'customer') {
      return res.status(403).json({
        success: false,
        message: "Tài khoản nhân viên/quản trị không thể thực hiện đặt lịch tại đây. Vui lòng sử dụng trang quản lý nội bộ.",
      });
    }

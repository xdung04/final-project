...
// Thay thế hàm getSalaries cũ bằng hàm này:
export const getSalaries = async (req, res) => {
  try {
    ...
    // Gọi hàm mới: Service sẽ tự quyết định lấy từ DB hay tính Real-time
    const salaries = await salaryService.getSalariesUnified(
      parseInt(month),
      parseInt(year)
    );
...
// ====================== 8. Thợ xem danh sách phiếu lương ======================
export const getMyPayslips = async (req, res) => {
  try {
    // Giả sử middleware check token của ông nhét thông tin user vào req.user
    const idUser = req.user.idUser; 

    const payslips = await salaryService.getMyPayslips(idUser);
    return res.status(200).json(payslips);
  } catch (err) {
    console.error("Lỗi getMyPayslips:", err);
    return res.status(400).json({ error: err.message });
  }
};

// ====================== 9. Thợ bấm nút "Xác nhận" ======================
export const confirmMyPayslip = async (req, res) => {
  try {
    ...
    const result = await salaryService.confirmPayslipByBarber(idSalary, idUser);
    return res.status(200).json({ message: "Đã xác nhận phiếu lương thành công", data: result });
  } catch (err) {
    console.error("Lỗi confirmMyPayslip:", err);
    return res.status(400).json({ error: err.message });
  }
};

// ====================== 10. Thợ bấm nút "Báo cáo sai sót / Khiếu nại" ======================
export const disputeMyPayslip = async (req, res) => {
  try {
    ...
    if (!idSalary || !reason) {
      return res.status(400).json({ error: "Thiếu ID phiếu lương hoặc lý do khiếu nại" });
    }

    const result = await salaryService.disputePayslipByBarber(idSalary, idUser, reason);
    return res.status(200).json({ message: "Đã gửi khiếu nại thành công", data: result });
  } catch (err) {
    console.error("Lỗi disputeMyPayslip:", err);
    return res.status(400).json({ error: err.message });
  }
};